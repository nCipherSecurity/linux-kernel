...
static nfp_err fsl_create(nfp_cdev *cdev)
{
	...
	if (cdev->bar[cdev->active_bar] == NULL) {
		nfp_log(NFP_DBG1, "fsl_create: error: null FSL memory BAR[%d]",
			cdev->active_bar);
		return NFP_ENOMEM;
	}
	...
	nfp_log(NFP_DBG3, "fsl_create: exiting fsl_create active_bar: %d.",
		cdev->active_bar);
...
static nfp_err fsl_destroy(void *ctx)
{
	...
	if (cdev->bar[cdev->active_bar] != NULL) {
		...
		nfp_log(NFP_DBG1, "fsl_destroy: warning: no FSL BAR[%d] memory",
			cdev->active_bar);
	}
...
* 
*/
static int fsl_connection_status(nfp_cdev *cdev, int lock_flag,
				 nfp_err epd_status)
{
...
static void fsl_check_complete(nfp_cdev *cdev, nfp_err status)
{
	...
	*/
	...
	if (status == NFP_SUCCESS) {
		...
		nfp_log(NFP_DBG1,
			"fsl_check_complete: device check failed with code: 0x%x",
			status);
	}
}
...
static nfp_err fsl_isr(void *ctx, int *handled)
{
	...
	*handled = 0;
	...
	doorbell_cs = FROM_LE32_MEM(&doorbell_cs);
	nfp_log(NFP_DBG3, "%s: cs:= %x,rd:=%x,wr:=%x", __func__, doorbell_cs,
		doorbell_rd, doorbell_wr);
	...
	while (doorbell_rd || doorbell_wr || doorbell_cs) {
		...
		if ((doorbell_cs) &&
			...
			nfp_log(NFP_DBG1,
				"fsl_isr: illegal bits in doorbell_cs %x",
				doorbell_cs);
			*handled = 1;
			/* clear the register*/
		...
		if ((doorbell_rd) &&
			...
			nfp_log(NFP_DBG1,
				"fsl_isr: illegal bits in doorbell_rd %x",
				doorbell_rd);
			*handled = 1;
			/* clear the register*/
			fsl_outl(cdev, cdev->active_bar,
		...
		if ((doorbell_wr) &&
			...
			nfp_log(NFP_DBG1,
				"fsl_isr: illegal bits in doorbell_wr %x",
				doorbell_wr);
			/* clear the register*/
			fsl_outl(cdev, cdev->active_bar,
				 FSL_OFFSET_DOORBELL_WR_STATUS,
		...
		if (doorbell_wr) {
			...
			nfp_write_complete(
				...
				doorbell_wr & NFAST_INT_DEVICE_WRITE_OK ? 1 :
									  0);
			nfp_log(NFP_DBG3,
				"fsl_isr: acknowledging write interrupt: ok = %d",
				doorbell_wr & NFAST_INT_DEVICE_WRITE_OK ? 1 :
									  0);
			...
			if (fsl_inl(cdev, cdev->active_bar,
			    ...
			    NFAST_INT_DEVICE_CLR) {
				nfp_log(NFP_DBG1,
					"fsl_isr: failed to clear doorbell write status");
			}
			++x_wr;
#endif
		}
		...
		if (doorbell_rd) {
			...
			nfp_log(NFP_DBG3,
				"fsl_isr: acknowledging read interrupt: ok = %d",
				doorbell_rd & NFAST_INT_DEVICE_READ_OK ? 1 : 0);
			...
			if (fsl_inl(cdev, cdev->active_bar,
			    ...
			    NFAST_INT_DEVICE_CLR) {
				nfp_log(NFP_DBG1,
					"fsl_isr: failed to clear doorbell read status");
			}
			++x_rd;
#endif
		}
		...
		if (doorbell_cs) {
			fsl_outl(cdev, cdev->active_bar,
				 ...
				 NFAST_INT_DEVICE_CLR);
			fsl_check_complete(
				cdev, doorbell_cs & NFAST_INT_DEVICE_CHECK_OK ?
					      NFP_SUCCESS :
					      NFP_ESTARTING);
			nfp_log(NFP_DBG3,
				"fsl_isr: acknowledging check interrupt: status:0x%x",
				doorbell_cs & NFAST_INT_DEVICE_CHECK_OK ?
					NFP_SUCCESS :
					NFP_ESTARTING);
			...
			if (fsl_inl(cdev, cdev->active_bar,
			    ...
			    NFAST_INT_DEVICE_CLR) {
				nfp_log(NFP_DBG1,
					"fsl_isr: warning: failed to clear doorbell check status");
			}
			++x_cs;
#endif
		}
		...
		nfp_log(NFP_DBG3, "fsl_isr: cs status in isr is: %x",
			doorbell_cs);
	}
	...
	*handled = 1;
	...
	if (x_wr + x_rd + x_cs == 0) {
		nfp_log(NFP_DBG2,
			"fsl_isr: no operations handled by this ISR call");
	} else {
		if (x_wr + x_rd + x_cs > 1) {
			nfp_log(NFP_DBG3,
				"fsl_isr: DEBUG: multiple operations handled by this ISR call: wr=%d, rd=%d, cs=%d",
				x_wr, x_rd, x_cs);
		} else {
			nfp_log(NFP_DBG3,
				"fsl_isr: DEBUG: one operation handled by this ISR call: wr=%d, rd=%d, cs=%d",
				x_wr, x_rd, x_cs);
		}
	}
#endif
 ...
 */
static nfp_err fsl_ensure_reading(int unsigned addr, int len, void *ctx,
				  int lock_flag)
{
	nfp_cdev *cdev;
	uint32_t hdr[3];
	...
	nfp_log(NFP_DBG3, "fsl_ensure_reading: cdev->bar[cdev->active_bar]= %x",
		cdev->bar[cdev->active_bar]);

	/* send read request */

	if (addr) {
		nfp_log(NFP_DBG3,
			"fsl_ensure_reading: requesting DMA reply to bus address %x",
			addr);
	...
	if ((ne = nfp_copy_to_dev(cdev, cdev->active_bar, NFPCI_JOBS_RD_CONTROL,
				  (char const *)hdr, hdr_len)) != NFP_SUCCESS) {
		nfp_log(NFP_DBG1,
			"fsl_ensure_reading: error: nfp_copy_to_dev failed");
		cdev->stats.ensure_fail++;
		return ne;
	}
	...
	if ((ne = nfp_copy_from_dev(cdev, cdev->active_bar,
				    NFPCI_JOBS_RD_LENGTH, (char *)hdr, 4)) !=
	    NFP_SUCCESS) {
		nfp_log(NFP_DBG1,
			"fsl_ensure_reading: error: nfp_copy_from_dev failed");
		cdev->stats.ensure_fail++;
		return ne;
	}
	...
	if (hdr[0] != tmp32) {
		nfp_log(NFP_DBG1,
			"fsl_ensure_reading: error: expected length not written (%08x != %08x)",
			hdr[0], tmp32);
		cdev->stats.ensure_fail++;
		return NFP_EIO;
	}
...
static nfp_err fsl_read(char *block, int len, void *ctx, int *rcnt)
{
	...
	*rcnt = 0;
	...
	if ((ne = nfp_copy_from_dev(cdev, cdev->active_bar,
				    NFPCI_JOBS_RD_LENGTH, (char *)&cnt, 4)) !=
	    NFP_SUCCESS) {
		cdev->stats.read_fail++;
	...
	if (cnt < 0 || cnt > len) {
		cdev->stats.read_fail++;
		nfp_log(NFP_DBG1,
			"fsl_read: error: bad byte count (%d) from device",
			cnt);
		return NFP_EIO;
	}
	...
	if ((ne = nfp_copy_to_user_from_dev(cdev, cdev->active_bar,
 ...
 */
static nfp_err fsl_write(int unsigned addr, char const *block, int len,
			 void *ctx)
{
	nfp_cdev *cdev;
	unsigned int hdr[3];
	...
	if (0 == addr) {
		...
		nfp_log(NFP_DBG3, "fsl_write: cdev->bar[cdev->active_bar]= %x",
			cdev->bar[cdev->active_bar]);
		nfp_log(NFP_DBG3, "fsl_write: block len %d", len);

		/* send write request */

		if ((ne = nfp_copy_from_user_to_dev(cdev, cdev->active_bar,
						    NFPCI_JOBS_WR_DATA, block,
						    len)) != NFP_SUCCESS) {
			cdev->stats.write_fail++;
			nfp_log(NFP_DBG1,
				"fsl_write: error: nfp_copy_from_user_to_dev failed");
			return ne;
		}

		TO_LE32_MEM(&hdr[0], NFPCI_JOB_CONTROL);
		TO_LE32_MEM(&hdr[1], len);
		if ((ne = nfp_copy_to_dev(
			     cdev, cdev->active_bar, NFPCI_JOBS_WR_CONTROL,
			     (char const *)hdr, 8)) != NFP_SUCCESS) {
			cdev->stats.write_fail++;
			nfp_log(NFP_DBG1,
				"fsl_write: error: nfp_copy_to_dev failed");
			return ne;
		}
		...
		if ((ne = nfp_copy_from_dev(cdev, cdev->active_bar,
					    NFPCI_JOBS_WR_LENGTH, (char *)hdr,
					    4)) != NFP_SUCCESS) {
			cdev->stats.write_fail++;
			nfp_log(NFP_DBG1,
				"fsl_write: nfp_copy_from_dev failed");
			return ne;
		}
		...
		if (hdr[0] != tmp32) {
			cdev->stats.write_fail++;
			nfp_log(NFP_DBG1,
				"fsl_write: length not written (%08x != %08x)",
				hdr[0], tmp32);
			return NFP_EIO;
		}
	} else {
		...
		nfp_log(NFP_DBG3, "fsl_write: cdev->bar[cdev->active_bar]= %x",
			cdev->bar[cdev->active_bar]);
		nfp_log(NFP_DBG3, "fsl_write: block len %d", len);
		nfp_log(NFP_DBG3, "fsl_write: pull from 0x%016x using DMA",
			addr);
